...
class MujocoBDX:
    ...
    def give_a_push(self, body_name: str, force: np.ndarray = [0,0,0], torque: np.ndarray = [0,0,0]) -> None:
        """
        Applies a force to the robot

        Args:
            force (np.ndarray): force to apply [x, y, z] in N
            body_name (str): body element on which the external force is applied
        """
